...
import com.benone.mvvm2.base.paging.BasePagingAdapter
import com.benone.mvvm2.model.Article
import com.benone.mvvm2.model.Banner
import com.benone.mvvm2.utils.ImageLoader
...
class HomeAdapter(retryCallback:()->Unit): BasePagingAdapter<Article>(diffCallback,retryCallback) {
    ...
    override fun bind(holder: ViewHolder, item: Article, position: Int) {
        ...
        if(item.envelopePic.isNotBlank()){
            context.let{
                ImageLoader.load(it,item.envelopePic,holder.getView(R.id.thumbnail) as ImageView)
            }
        }
